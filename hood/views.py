...
def hood(request,neighbourhood_id):
  ...
  try:
    hood = Neighbourhood.objects.filter(id=neighbourhood_id)
    found = Business.objects.filter(neighbourhood=neighbourhood_id)[0:1]
  except DoesNotExist:
    raise Http404() 
...
def new_neighbour(request):
  ...
  if request.method == 'POST':
    ...
    if form.is_valid():
      ...
      Profile.objects.filter(user=current_user).update(prof_pic=prof_pic,name=name,email=email)
      profile.save()       
